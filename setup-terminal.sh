...
[ -f /home/"$USER"/GIT/dotfiles ] || {
    echo "~/GIT/dotfiles does not exist"
    exit 1
}

apt install zsh go ninja-build gettext cmake unzip curl dash
ln -sfT /bin/dash /bin/sh >/dev/null 2>&1
mkdir -p /home/"$USER"/.cache/zsh/

#LF
su $USER -c sh "env CGO_ENABLED=0 go install -ldflags="-s -w" github.com/gokcehan/lf@latest"
mv /home/"$USER"/.local/go/share/lf /usr/bin/ && chown root:root /usr/bin/lf
go clean
#NEOVIM
