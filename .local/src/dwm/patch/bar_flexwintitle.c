...
#define FLEXWINTITLE_FLOATWEIGHT 4   // floating window title weight, set to 0 to not show floating windows
#endif
...
getschemefor(Monitor *m, int group, int activegroup)
{
	...
	case GRP_STACK2:
		...
		if (m->lt[m->sellt]->arrange == &centeredmaster)
			return (activegroup ? SchemeFlexActCM : SchemeFlexInaCM);
		#endif // CENTEREDMASTER_LAYOUT
		#if CENTEREDFLOATINGMASTER_LAYOUT
		if (m->lt[m->sellt]->arrange == &centeredfloatingmaster)
			return (activegroup ? SchemeFlexActCFM : SchemeFlexInaCFM);
		...
		if (m->lt[m->sellt]->arrange == &deck) {
			return (activegroup ? SchemeFlexActDECK : SchemeFlexInaDECK);
		}
		#endif // DECK_LAYOUT
...
flextitledraw(Monitor *m, Client *c, int unused, int x, int w, int tabscheme, Arg *arg)
{
	...
	if (FLEXWINTITLE_BORDERS) {
		...
		XFillRectangle(drw->dpy, drw->drawable, drw->gc, x, 0, 4, bh);
		XFillRectangle(drw->dpy, drw->drawable, drw->gc, x + w, 0, 4, bh);
	}
	/* Optional tags icons */
	for (i = 0; i < NUMTAGS; i++) {
